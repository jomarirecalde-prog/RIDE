...
$pageTitle = ($isEdit ? 'Edit' : 'New') . ' Consolidated Ongoing Researches — RIDE IMS';

$pageHeading = 'Consolidated Ongoing Researches';

$isVprideConsolidation = ($isVprideConsolidation ?? false)
    || ($isEdit && proposal_is_vpride_consolidated_ongoing_researches($proposal));

$pageSubtitle = $isEdit

    ? 'Update the consolidated ongoing researches report before saving or resubmitting.'

    : ($isVprideConsolidation
        ? 'University-wide consolidation of faculty ongoing researches approved by the Director of Research (WPU-QSF-RIDE-RDO-06).'
        : 'College consolidation of faculty ongoing researches approved by the Director of Research (WPU-QSF-RIDE-RDO-06).');
...
if (
    \App\Services\OngoingResearchesConsolidation::entriesHaveRows($prefilledEntries)
    && !\App\Services\OngoingResearchesConsolidation::entriesHaveRows($entries)
) {
...
<form class="proposal-paper completed-researches-paper consolidated-ongoing-researches-paper" method="post" action="<?= $isEdit ? base_url('proposals/' . $proposal['id'] . '/consolidated-ongoing-researches') : base_url('proposals/consolidated-ongoing-researches') ?>">
    ...
    <header class="completed-researches-header">
        ...
        <h2 class="completed-researches-title">CONSOLIDATED ONGOING RESEARCHES</h2>

        <p class="completed-researches-form-id">WPU-QSF-RIDE-RDO-06 Rev.00 (09.15.25)</p>
            ...
            <tr>
                ...
                <td>
                    ...
                    <?php else: ?>
                        ...
                        <input type="hidden" name="college_name" id="consolidated-ongoing-researches-college-name" value="">
    ...
    <?php if (\App\Services\OngoingResearchesConsolidation::entriesHaveRows($entries)): ?>
        <p class="proposal-section-note"><?= $isVprideConsolidation
            ? 'Rows below include faculty ongoing researches from all colleges, deposited after Director of Research approval. The Category column shows the college.'
            : 'Rows below include faculty ongoing researches for your college, deposited after Director of Research approval.' ?></p>
    <?php elseif (!$isEdit): ?>
        <p class="proposal-section-note">No faculty ongoing researches approved by the Director of Research were found yet. They are added automatically when the Director of Research approves each report.</p>
    <?php endif; ?>
    ...
    $workflowProposal = $proposal ?? [
        ...
        'summary' => json_encode(['form_type' => 'consolidated_ongoing_researches']),
...
<?php if (!$isEdit && $entries !== []): ?>

    <p class="proposal-section-note">Rows from approved faculty ongoing researches are pre-filled. Save a draft to keep them in your consolidated report.</p>

<?php endif; ?>

<template id="consolidated-ongoing-researches-row-template">
...
(() => {
    ...
    const collegeNameInput = document.getElementById('consolidated-ongoing-researches-college-name');
    ...
    const rowTemplate = document.getElementById('consolidated-ongoing-researches-row-template');
