...
<section class="proposal-section trainings-conducted-documents">
    <h3 class="proposal-subtitle">Appendices</h3>
    <p class="proposal-section-note">
        Provide a caption in each appendix. Number captions in the same sequence as figures.
        Attach: group picture; training team picture; pictures during activities; approved request letter;
        travel order; certificates (participant participation, appreciation from client/partner, recognition for the training team);
        attendance sheet; program; IEC materials; and one sample of Ebalwasyon ng Gawain.
    ...
    <table class="proposal-table trainings-conducted-documents-table">
        ...
        <tbody>
            <?php foreach ($requiredFileList as $fileKey => $fileName): ?>
                <?php if (in_array($fileKey, ['ar_results_table', 'ar_results_figure'], true)) { continue; } ?>
