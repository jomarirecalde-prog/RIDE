...
<section class="proposal-section completed-researches-section research-output-published-section trainings-conducted-section" data-funding-section="<?= htmlspecialchars($sectionKey) ?>">
    ...
    <div class="proposal-table-wrap trainings-conducted-table-wrap research-output-published-table-wrap">
        <table class="proposal-table research-output-published-table trainings-conducted-table">
            ...
            <thead>
                <tr>
                    <th colspan="4">1. Program/Project/Study</th>
                    <th colspan="8">2. Research Output Published</th>
                    <?php if (!$readOnly): ?>
                ...
                <tr>
                    ...
                    <th>Date Completed (mm/dd/yyyy)</th>
                    <th>Duration in Months</th>
                    <th>Author(s)/ Researcher(s)</th>
                    <th>Title of Article</th>
                    <th>Title of Journal/ Book</th>
                    <th>Editor(s)</th>
            ...
            <tbody class="research-output-published-rows" data-section="<?= htmlspecialchars($sectionKey) ?>">
                <?php foreach ($rows as $index => $row): ?>
                    <tr class="research-output-published-row">
                        ...
                        <td>
                            ...
                            <?php else: ?>
                                <textarea
                                    name="entries[<?= htmlspecialchars($sectionKey) ?>][<?= (int) $index ?>][authors_researchers]"
                                    class="proposal-textarea proposal-textarea-compact"
                                    rows="2"
                                    placeholder="Author(s)/ researcher(s)"
                                ><?= htmlspecialchars($entryValue($row, 'authors_researchers')) ?></textarea>
                            <?php endif; ?>
                        </td>
                        <td>
                            <?php if ($readOnly): ?>
                            ...
                            <?php else: ?>
                                <textarea
                                    name="entries[<?= htmlspecialchars($sectionKey) ?>][<?= (int) $index ?>][article_title]"
                                    class="proposal-textarea proposal-textarea-compact"
                                    rows="2"
                                    placeholder="Title of article"
                                ><?= htmlspecialchars($entryValue($row, 'article_title')) ?></textarea>
                            <?php endif; ?>
                        </td>
                        <td>
                            <?php if ($readOnly): ?>
                            ...
                            <?php else: ?>
                                <textarea
                                    name="entries[<?= htmlspecialchars($sectionKey) ?>][<?= (int) $index ?>][journal_book_title]"
                                    class="proposal-textarea proposal-textarea-compact"
                                    rows="2"
                                    placeholder="Title of journal/book"
                                ><?= htmlspecialchars($entryValue($row, 'journal_book_title')) ?></textarea>
                            <?php endif; ?>
                        </td>
                        <td>
                            <?php if ($readOnly): ?>
                                <?= htmlspecialchars($entryValue($row, 'editors') !== '' ? $entryValue($row, 'editors') : '—') ?>
                            <?php else: ?>
                                <input
                                    type="text"
                                    name="entries[<?= htmlspecialchars($sectionKey) ?>][<?= (int) $index ?>][editors]"
                                    value="<?= htmlspecialchars($entryValue($row, 'editors')) ?>"
                        ...
                        <td>
                            ...
                            <?php else: ?>
                                <input
                                    type="text"
                                    name="entries[<?= htmlspecialchars($sectionKey) ?>][<?= (int) $index ?>][volume_issue]"
                                    value="<?= htmlspecialchars($entryValue($row, 'volume_issue')) ?>"
                        ...
                        <td>
                            ...
                            <?php else: ?>
                                <input
                                    type="text"
                                    name="entries[<?= htmlspecialchars($sectionKey) ?>][<?= (int) $index ?>][pages]"
                                    value="<?= htmlspecialchars($entryValue($row, 'pages')) ?>"
                                    placeholder="Pages"
                                >
                            <?php endif; ?>
                        </td>
                        <td>
                            ...
                            <?php else: ?>
                                <input
                                    ...
                                    max="2100"
                                    step="1"
                                    name="entries[<?= htmlspecialchars($sectionKey) ?>][<?= (int) $index ?>][publication_year]"
                                    value="<?= htmlspecialchars($entryValue($row, 'publication_year')) ?>"
                                    placeholder="Year"
                                >
                            <?php endif; ?>
                        </td>
                        <td>
                            <?php if ($readOnly): ?>
                                <?= nl2br(htmlspecialchars($entryValue($row, 'indexing') !== '' ? $entryValue($row, 'indexing') : '—')) ?>
                            <?php else: ?>
                                <textarea
                                    name="entries[<?= htmlspecialchars($sectionKey) ?>][<?= (int) $index ?>][indexing]"
                                    class="proposal-textarea proposal-textarea-compact"
                                    rows="2"
                                    placeholder="Indexing"
                                ><?= htmlspecialchars($entryValue($row, 'indexing')) ?></textarea>
                            <?php endif; ?>
                        </td>
                        <?php if (!$readOnly): ?>
    ...
    <?php if (!$readOnly): ?>
        <div class="proposal-coauthors-actions completed-researches-add-row-wrap">
            <button type="button" class="btn btn-sm btn-outline research-output-published-add-row" data-section="<?= htmlspecialchars($sectionKey) ?>">
